...
export default async function (message: Discord.Message, args: string[]) {
    if (!message.guild) return await message.channel.send(errorBuilder("You can't use this command in DM! Please ping me instead", "setprefix", message));
    let newPrefix = args[0];
    if (!newPrefix)
        return await message.channel.send(
            errorBuilder('Please provide a prefix!', 'setPrefix', message)
        );
    if (newPrefix.length > 5)
        return await message.channel.send(
            errorBuilder('Prefix must be less than 5 characters!','setPrefix',message)
        );
    if (newPrefix.length > 1){
        await message.channel.send({embeds:[new Discord.MessageEmbed().setDescription(`Warning, prefix will be set with a trailing space. Ex: ${newPrefix} <Command>`).setColor("#ff0000")]});
        newPrefix = newPrefix + " ";
