...
export default async function (message: Discord.Message, args:any[] ) {
 ...
 for (let i = 0; i < args.length; i++) {
  ...
  let send = await message.channel.send({content:`${message_to_send}, you are now playing a game of what the is that word. React to this message to begin`});
...
//something here
  ...
  while (true){
    ...
    try{
    let answers = await message.channel.awaitMessages({ filter:messageFilter, max:list_of_players.length, time: 2000, errors: ['time'] })
    if (!answers) {continue}
    //TODO: make it so that check list if empty instead of just putting ! for safety
    if (answers.every(message=> message.content.toLowerCase() == answers.first()!.content.toLowerCase())){
   
      await message.channel.send(`congrats lol. the word was ${answers.first()!.content}`)
    break
    }
  }
    catch(e){
